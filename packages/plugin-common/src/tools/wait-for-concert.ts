...
export async function waitForConcert(
  ...
  if (initial.status !== 'running' && initial.status !== 'pending') {
    onUpdate?.(`Concert already finished with status: ${initial.status}.`);
  ...
  while (true) {
    ...
    const events = await (orchestron.liveEventLog?.read(input.concertId) ??
      orchestron.store.getEvents(input.concertId, {
    ...
    if (!current) {
      onUpdate?.(`Concert ${input.concertId} no longer exists.`);
      break;
    }
